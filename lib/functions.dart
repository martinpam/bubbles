...
double y_range(int speed, double x_distance, double y_distance) {
  double secondsToReachTarget =
      (hypotenuse(x_distance, y_distance) * 100 / speed);
...
double x_range(int speed, double x_distance, double y_distance) {
  double secondsToReachTarget =
...
getSecondsToReachTarget(double x_distance, double y_distance, int speed) {
  return hypotenuse(x_distance, y_distance) * 100 / speed;
}
...
List<double> calculateDistances(var entry) {
  ...
  if (x_posSpawn < 0 && x_posTarget < 0 ||
      x_posSpawn >= 0 && x_posTarget >= 0) {
    x_posSpawn *= -1;
  }
  if (y_posSpawn < 0 && y_posTarget < 0 ||
      y_posSpawn >= 0 && y_posTarget >= 0) {
    y_posSpawn *= -1;
  }
...
void updatePositionsNew(
    var entry, List<double> distances, int intensity, double currentTime) {
